...
export default function Men(): JSX.Element {
    ...
    // 종류 버튼 가져오기
    function getList() {
        axios.get(BUTTON_API_URL).then((res: any) => {
            console.log(res);
    ...
    // 신발 데이터 가져오기
    function getData() {
        axios.get(Men_API_URL).then((res: any) => {
            console.log(res);
    ...
    // function nameFilter(){
    //     let newBest = [...data];
    //     newBest.sort(function(a: {name:string},b: {name: string}) {
    //         if(a.name < b.name){
    //             return -1;
    //         }
    //         if(a.name == b.name){
    ...
    function lowPriceFilter(){
        let newBest = [...data];
        newBest.sort(function(a: { price: number; }, b: { price: number; }){
                ...
                <div className="flex items-center lg:w-screen min-h-screen my-16">
                    <div className="container m-auto flex flex-wrap items-start">
                        <div className="w-full pl-0 lg:pl-2 mb-0 lg:mb-4 mt-4 m-auto text-center">
                            ...
                            <button onClick={lowPriceFilter}>dfd</button>
                        ...
                        <div className='w-full text-center mt-7 lg:mt-5 mx-auto mb-7 lg:mb-0'>
                            {
                                list.map(function (a: {name: string, number: number}, i: number | string) {
                                    return (
                                        <button
                                            value={i}
                        ...
                        {
                            data.map(function (a: SlideType, i: number) {
                                return (
                                    <>
                                        <div className="w-1/2 lg:w-1/3 pl-0 md:pl-5 lg:pl-2 mt-16 lg:pr-2 cursor-pointer" key={a?.index} onClick={() => router.push(`/view/${a?.index}`)}>
