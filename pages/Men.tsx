...
export default function Men(): JSX.Element {
    ...
    // 신발 데이터 가져오기
    function getData() {
        axios.get(Men_API_URL).then((res) => {
            console.log(res);
    ...
    useEffect(() => {
        ...
        if (localStorage.getItem(`Men_pageNum`) != null) {
            setPage(parseInt(`${localStorage.getItem(`Men_pageNum`)}`));
        }
    }, []);

    useEffect(() => {
        const state = localStorage.getItem("Men_StateInLocal");
        if (state) {
                        ...
                        <div className="container m-auto flex flex-wrap items-start">
                            <div className="w-full pl-0 lg:pl-2 mb-0 lg:mb-4 mt-4 m-auto text-center">
                                ...
                                <div className='inline-flex lg:flex items-center mx-auto mt-8 lg:mt-0 lg:float-right'>
                                    ...
                                    <Dropdowns data={data} setData={setData} Name={"Men"}></Dropdowns>
                                </div>
                            </div>
