...
export default function Detail(): JSX.Element {
    ...
    function Before() {
        if (ID == 0) {
            return (
                <Link href="#void" className="w-1/2 bg-white dark:bg-slate-900 shadow text-left p-6 cursor-auto">
            ...
            return (
                <Link href={"/Detail/" + (parseInt(ID) - 1)} onClick={() => { scrollTo() }} className="w-1/2 bg-white dark:bg-slate-900 shadow hover:shadow-md text-left p-6">
                    <p className="text-lg text-blue-800 font-bold flex items-center"><FontAwesomeIcon icon={faArrowLeft} className="pr-2" /> 이전 기사</p>
    ...
    function Next() {
        if ((parseInt(ID) + 1) == list.length) {
            return (
                <Link href="#void" className="w-1/2 bg-white dark:bg-slate-900 shadow text-right p-6 cursor-auto">
                    <p className="text-lg text-blue-800 font-bold flex items-center justify-end line-clamp-1">다음 기사 <FontAwesomeIcon icon={faArrowRight} className="pl-2" /></p>
                    <p className="pt-2 line-clamp-1">더 이상의 기사가 없습니다.</p>
                </Link>
            )
        } else {
            return (
                <Link href={"/Detail/" + (parseInt(ID) + 1)} onClick={() => { scrollTo() }} className="w-1/2 bg-white dark:bg-slate-900 shadow hover:shadow-md text-right p-6">
                    <p className="text-lg text-blue-800 font-bold flex items-center justify-end line-clamp-1">다음 기사 <FontAwesomeIcon icon={faArrowRight} className="pl-2" /></p>
                    <p className="pt-2 line-clamp-1">{list[parseInt(ID) + 1]?.h2}</p>
                </Link>
            )
