...
import { SlideType } from './Type/TypeBox';
...
export default function Women(): JSX.Element {
    ...
    // 신발 데이터 가져오기
    function getData() {
        axios.get(Women_API_URL).then((res) => {
            console.log(res);
    ...
    useEffect(() => {
        ...
        if (localStorage.getItem(`Women_pageNum`) != null) {
            setPage(parseInt(`${localStorage.getItem(`Women_pageNum`)}`));
        }
    }, []);

    useEffect(() => {
        const state = localStorage.getItem("Women_StateInLocal");
        if (state) {
                        ...
                        <div className="container m-auto flex flex-wrap items-start">
                            <div className="w-full pl-0 lg:pl-2 mb-0 lg:mb-4 mt-4 m-auto text-center">
                                ...
                                <div className='inline-flex lg:flex items-center mx-auto mt-8 lg:mt-0 lg:float-right'>
                                    ...
                                    <Dropdowns data={data} setData={setData} Name={"Women"}></Dropdowns>
                                </div>
                            </div>
                                            ...
                                            data && data.slice(offset, offset + limit).map(function (item: SlideType, idx: number) {
                                                return (
                                                    <React.Fragment key={idx}>
                                                        <div
                                                            className="w-1/2 lg:w-1/3 pl-0 md:pl-5 lg:pl-2 mb-16 lg:pr-2 cursor-pointer"
                                                                ...
                                                                <Fade bottom>
                                                                    <div className=" pt-5 px-2 flex flex-col gap-2">
                                                                        ...
                                                                        <div className='tracking-tighter'>
                                                                            ...
                                                                            <span className="text-base md:text-xl relative xl:absolute right-0 xl:right-3 translate-y-0 xl:-translate-y-16 mt-0">{item?.price}</span>
                                                                        </div>
